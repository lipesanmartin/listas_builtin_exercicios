# Utilizando listas faça um programa que faça 5 perguntas para uma pessoa sobre um crime. As perguntas são:
# "Telefonou para a vítima?"
# "Esteve no local do crime?"
# "Mora perto da vítima?"
# "Devia para a vítima?"
# "Já trabalhou com a vítima?"
#
# O programa deve no final emitir uma classificação sobre a participação da pessoa
# no crime. Se a pessoa responder positivamente a 2 questões ela deve ser classificada como "Suspeita",
# entre 3 e 4 como "Cúmplice" e 5 como "Assassino". Caso contrário, ele será classificado como "Inocente".

lista_perguntas = ["Telefonou para a vítima? ",
                   "Esteve no local do crime? ",
                   "Mora perto da vítima? ",
                   "Devia para a vítima? ",
                   "Já trabalhou com a vítima? "]

lista_condicoes = ["Suspeita", "Cúmplice", "Assassino", "Inocente"]

respostas = 0
print("Responda S ou N")
for x in range(len(lista_perguntas)):
    while True:
        pergunta = str(input(lista_perguntas[x]))
        if pergunta in 'nNsS':
            if pergunta in 'sS':
                respostas += 1
            break
        else:
            print("Resposta inválida.")
if respostas == 2:
    print(lista_condicoes[0])
elif respostas == 3 or respostas == 4:
    print(lista_condicoes[1])
elif respostas == 5:
    print(lista_condicoes[2])
else:
    print(lista_condicoes[3])
